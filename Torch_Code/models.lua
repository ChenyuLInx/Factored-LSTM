dofile 'util.lua'
-- This code is adapted from Yoon Kim's seq2seq-attn code on https://github.com/harvardnlp/seq2seq-attn.git
...
function make_factored_lstm(data, opt, model, use_chars)
  assert(model == 'enc' or model == 'dec')
  local name = '_' .. model
  local dropout = opt.dropout or 0
  local n = opt.num_layers
  local rnn_size = opt.rnn_size
  local factor_size = opt.factor_size
  local keyword_vec_size = opt.keyword_vec_size
  local input_size
  if use_chars == 0 then
    input_size = opt.word_vec_size
  else
  ...
  table.insert(inputs, nn.Identity()()) -- keyword vector
  local keyword_vec =  inputs[#inputs]
  ...
  for L = 1,n do
    ...
    -- calculate factored input and hidden state once for efficiency
    local k2fac_i = nn.LinearNoBias(keyword_vec_size, 4 * factor_size)(keyword_vec)
    local k2fac_h = nn.LinearNoBias(keyword_vec_size, 4 * factor_size):reuseMem()(keyword_vec)
    local i2fac = nn.LinearNoBias(input_size_L, 4 * factor_size):reuseMem()(x)
    local h2fac = nn.LinearNoBias(rnn_size, 4 * factor_size):reuseMem()(prev_h)
    local fac_i = nn.CMulTable()({k2fac_i, i2fac})
    local fac_h = nn.CMulTable()({k2fac_h, h2fac})

    local reshaped_fac_i = nn.Reshape(4, factor_size)(fac_i)
    local reshaped_fac_h = nn.Reshape(4, factor_size)(fac_h)
    local i_1, i_2, i_3, i_4 = nn.SplitTable(2)(reshaped_fac_i):split(4)
    local h_1, h_2, h_3, h_4 = nn.SplitTable(2)(reshaped_fac_h):split(4)
    local i2h_1 = nn.Linear(factor_size, rnn_size):reuseMem()(i_1)
    local i2h_2 = nn.Linear(factor_size, rnn_size):reuseMem()(i_2)
    local i2h_3 = nn.Linear(factor_size, rnn_size):reuseMem()(i_3)
    local i2h_4 = nn.Linear(factor_size, rnn_size):reuseMem()(i_4)
    local h2h_1 = nn.LinearNoBias(factor_size, rnn_size):reuseMem()(h_1)
    local h2h_2 = nn.LinearNoBias(factor_size, rnn_size):reuseMem()(h_2)
    local h2h_3 = nn.LinearNoBias(factor_size, rnn_size):reuseMem()(h_3)
    local h2h_4 = nn.LinearNoBias(factor_size, rnn_size):reuseMem()(h_4)
    -- decode the gates
    local in_gate = nn.Sigmoid():reuseMem()(nn.CAddTable()({i2h_1, h2h_1}))
    local forget_gate = nn.Sigmoid():reuseMem()(nn.CAddTable()({i2h_2, h2h_2}))
    local out_gate = nn.Sigmoid():reuseMem()(nn.CAddTable()({i2h_3, h2h_3}))
    -- decode the write inputs
    local in_transform = nn.Tanh():reuseMem()(nn.CAddTable()({i2h_4, h2h_4}))
    -- perform the LSTM update
    local next_c = nn.CAddTable()({
        nn.CMulTable()({forget_gate, prev_c}),
        nn.CMulTable()({in_gate, in_transform})
...
function keyword_generator(vecs, opt)
  local model = nn.Sequential()
  model:add(nn.Linear(800, 1200))
  model:add(nn.Linear(1200, vecs:size(1)))
