...
"""Create the data for the LSTM.
    This code is adapted from Yoon Kim's code on https://github.com/harvardnlp/seq2seq-attn.git
"""
...
class Indexer:
    ...
    def prune_vocab(self, k, n):
        ...
        if n == 1:
            keyword = open("keyword-11-13.txt", 'w')
...
def get_data(args):
    ...
    def convert(srcfile, targetfile, batchsize, seqlength, outfile, num_sents,
        ...
        if args.keyword_file != '':
            keyword_vecs = np.zeros((num_sents, seqlength), dtype=int)
        source_lengths = np.zeros((num_sents,), dtype=int)
        target_lengths = np.zeros((num_sents,), dtype=int)
        keyword_lengths = np.zeros((num_sents,), dtype=int)
        if chars==1:
        ...
        for _, (src_orig, targ_orig) in \
            ...
            targ_char = []
            if args.keyword_file != '':
                keyword_vec = np.zeros(seqlength, dtype=int)
            keyword_count = 0
            for word in targ:
                if chars == 1:
                    word = char_indexer.clean(word)
                ...
                if args.keyword_file != '':
                    if word in keywords:
                        keyword_vec[keyword_count] = keywords[word]
            ...
            if args.keyword_file != '':
                ...
                keyword_lengths[sent_id] = (keyword_vecs[sent_id] != 0).sum()
        ...
        if shuffle == 1:
            ...
            if args.keyword_file != '':
                ...
                keyword_lengths = keyword_lengths[rand_idx]
            if chars==1:
        ...
        if args.keyword_file != '':
            ...
            keyword_l = keyword_lengths[source_sort]
        ...
        for i in range(len(batch_idx)-1):
            ...
            if args.keyword_file != '':
                batch_keyword_l_max.append(max(keyword_l[batch_idx[i]-1:batch_idx[i+1]-1]))
        ...
        if args.keyword_file != '':
            f['vecs'] = keyword_vecs
            f['batch_keyword_l'] = batch_keyword_l_max
            f['keyword_size'] = np.array([len(keywords)])
        if chars == 1:
...
def main(arguments):
    ...
    # load keywords to construct keyword vector
    if args.keyword_file != '':
